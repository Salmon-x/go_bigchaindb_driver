...
func createInputsFromUnspentTransactions(unspentTransactions []Transaction) ([]Input, error) {
	...
	for _, ut := range unspentTransactions {
		...
		if ut.Operation == "CREATE" {
			assetIds = append(assetIds, *ut.ID)
		} else {
			assetIds = append(assetIds, *ut.Asset.ID)
		}
	}

	for i, uo := range unspentOutputs {
		input := Input{
			OwnersBefore: uo.PublicKeys,
		}
		ol := new(OutputLocation)
		ol.OutputIndex = i
		ol.TransactionID = assetIds[i]
		input.Fulfills = ol
		inputs = append(inputs, input)
...
func NewOutput(c condition, amount string) (Output, error) {
	if amount == "" {
		amount = "1"
	}
	...
	return Output{
		Condition: Condition{
			...
			Details: ConditionDetail{
				PublicKey: c.pub,
				Type:      strings.ToLower(c.fpt),
			},
		},
		Amount:     amount,
		PublicKeys: []string{c.pub},
	}, nil
}
...
func (b *bigChaindbTransaction) CreateTx() {
