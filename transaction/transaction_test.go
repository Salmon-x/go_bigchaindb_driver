...
func TestRemoveSignatures(t *testing.T) {
	tx := new(Transaction)
	fu := "qwqwe"
	tx.Inputs = append(tx.Inputs, Input{Fulfillment: &fu})
	tx._removeSignatures()

}

func TestNew(t *testing.T) {
	bs := []byte("\x81!\xd1\x94\x84\x87&\x19\xed\x16\xdd\xa6\xf5\xad\xaa\x938\x05L\xc8\xb2_\x97sX\xd2J\xfc\xdf\xbb\x8fJ")
	fmt.Println(base58.Encode(bs))
}

func TestCreateConition2(t *testing.T) {
	//aliceKey := "HKyUqBRMWcXxtinkeLETgftmJbNmkJnTC9kh6q2YwmdW"
	bobKey := "CxZo9jZtm69GzgiCFdBB5L6kpLAHscm9xgaaV8HtLVfL"
	//ak := base58.Decode(aliceKey)
	bk := base58.Decode(bobKey)
	c, err := NewCondition(bk)
	if err != nil {
		return
	}
	ff := "ni:///sha-256;hOkq6jB9Cn6YNB1ZILG1fZqh1J3COPUflw7t1V1Ks88?fpt=ed25519-sha-256&cost=131072"
	fmt.Println(ff)
	fmt.Println(c.uri)
	fmt.Println(ff == c.uri)
}
